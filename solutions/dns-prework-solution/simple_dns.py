...
ResourceRecord = namedtuple(
    'ResourceRecord',
    'name type dns_class ttl rdlength rdata'
)
DNSMessage = namedtuple(
    'DNSMessage',
...
def _format_header(r):
    return f""";; ->>HEADER<<- id: {r.xid}
;; flags: {r.flags:b}; QUERY: {r.qdcount}, ANSWER: {r.ancount}, AUTHORITY: {r.nscount}, ADDITIONAL: {r.arcount}"""  # NOQA


def _format_question(r):
    return f'{r.qname}\t\t\tIN\t{r.qtype}'


def _format_record(r):
    return f'{r.name}\t\t{r.ttl}\tIN\t{TYPE_NAMES[r.type]}\t{r.rdata}'
...
DNSMessage.__repr__ = _format_message


def construct_query(hostname, record_type):
    """
    Construct a query message for the given name and record type.

    Use a random transaction id, set only the RD ("recursive desired")
    flag, and indicate that we have one question and no other records.
    """
...
def decode(bs):
    """
    Decode a `DNSMessage` by parsing the given bytes
    """
    ...
    for n, records in sections:
        for _ in range(n):
            name, idx = parse_name(bs, idx)
            rtype, rclass, ttl, length = \
                struct.unpack('!HHIH', bs[idx:idx+10])
            idx += 10
...
def encode(msg):
    ...
    name = b''.join(len(p).to_bytes(1, 'big') + bytes(p, 'ascii')
                    for p in msg.questions[0].qname.split('.'))
    return struct.pack(
...
def parse_name(bs, i):
    ...
    while True:
        ...
        if b >> 6 == 0b11:  # if first two bits are on, it's a pointer
            ...
            return '.'.join([str(l, 'ascii')
                            for l in labels] + [result]), i + 2
...
if __name__ == '__main__':
    ...
    query = construct_query(name, record_type)
    sock.sendto(encode(query), GOOGLE_PUBLIC_DNS)
